...
export async function POST(req: NextRequest) {
  try {
    ...
    if (advisorError) {
      ...
      return NextResponse.json({ error: 'Failed to fetch advisor information' }, { status: 500 });
    }
...
return NextResponse.json({
  success: true,
  message: `Successfully sent ${sent} emails${failed > 0 ? `, ${failed} failed` : ''}`,
  results: results.map(r => r.status === 'fulfilled' ? r.value : { email: r.reason.email, success: false, error: r.reason.error })
});
```