...
import { singleMessagetoSingleUser } from "@/app/api/whatsapp/route";
...
export default function PostAdviceForm({
  ...
  const [segment, setSegment] = useState<"EQUITY" | "F&O" | "COMMODITIES">(
    "EQUITY"
  );
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      ...
      if (!error && data) {
        const uniquePlans = [
          ...new Set(data.map((client) => client.plan).filter(Boolean)),
        ];
    ...
    const fetchSearchResults = async () => {
      if (searchQuery.length > 2) {
        ...
        const { data, error } = await supabase
          .from("client3")
          .select(
            "id, name, email, whatsapp, assigned_rn, risk, ekyc_status, plan"
          )
          .or(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      singleMessagetoSingleUser(
          selectedClient.whatsapp,
          getDefaultMessage(selectedClient)
        );
  ...
  return (
    <Card
      className="max-w-lg w-full border rounded-xl shadow-lg bg-white"
      ref={searchRef}
    >
          ...
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <RadioGroup
                  value={segment}
                  onValueChange={(value: "EQUITY" | "F&O" | "COMMODITIES") =>
                    setSegment(value)
                  }
              ...
              <div>
                ...
                <RadioGroup
                  value={timeHorizon}
                  onValueChange={(value: "INTRADAY" | "SWING" | "LONGTERM") =>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <div className="flex items-center gap-2">
                  <Switch
                    checked={rangeTarget}
                    onCheckedChange={setRangeTarget}
                  />
                  <Label className="cursor-pointer">Range</Label>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {selectedPlan && planClients.length > 0 && (
                    <div className="mt-2 text-sm text-gray-600">
                      {planClients.length} clients with{" "}
                      {selectedPlan.toUpperCase()} plan
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    This will open WhatsApp with a pre-filled message that you
                ...
                {selectedClient && (
                  <div className="mt-3 p-3 border rounded bg-white">
                      ...
                      <div>
                        ...
                        {selectedClient.ekyc_status && (
                          <Badge
                            className="ml-2"
                            variant={
                              selectedClient.ekyc_status === "verified"
                                ? "outline"
                                : "destructive"
                            }
                          >
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      <Badge
                        variant={
                          selectedClient.risk === "aggressive"
                            ? "destructive"
                            : "outline"
                        }
                      >
                      ...
                      <Badge variant="outline">
                        {selectedClient.assigned_rn || "No RN"}
                      </Badge>
                    </div>
                  </div>
                )}
