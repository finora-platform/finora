...
export const EditTradeModal = ({
  ...
  const getWhatsAppMessage = () => {
    ...
    const timeHorizonText =
      formValues.timeHorizon.charAt(0) +
      formValues.timeHorizon.slice(1).toLowerCase();
    ...
    return (
      ...
      `Entry: ${entryText}\n` +
      `Stop Loss: ${formValues.stoploss}${
        formValues.trailingSL ? " (Trailing)" : ""
      }\n` +
      `${targetText}\n\n` +
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (existingData) {
        ...
        result = await supabase.from("user_trades").insert([
          {
            user_id: clientId,
      ...
      const client = clientInfo || clientData;
      if (client?.whatsapp) {
        singleMessagetoSingleUser(client.whatsapp, getWhatsAppMessage());
      }
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-[480px] rounded-xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-lg font-semibold">
            ...
            <span className="text-xs border px-1.5 py-0.5 rounded bg-muted">
              {trade.segment}
            </span>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-2">
          ...
          <RadioGroup
            ...
            onValueChange={(val) =>
              setFormValues((prev) => ({ ...prev, timeHorizon: val }))
            }
        ...
        <div className="mt-4 space-y-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) =>
                setFormValues((prev) => ({ ...prev, entry: e.target.value }))
              }
            />
            {entryRange && (
              <Input
        ...
        <div className="mt-4 space-y-1">
          ...
          <div className="grid grid-cols-2 gap-4 items-center">
            <Input
              ...
              onChange={(e) =>
                setFormValues((prev) => ({ ...prev, stoploss: e.target.value }))
              }
            />
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              Trailing % (of LTP)
              <Switch
                ...
                onCheckedChange={(checked) =>
                  setFormValues((prev) => ({ ...prev, trailingSL: checked }))
                }
              />
            </div>
          </div>
        ...
        <div className="flex justify-end mt-6">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <span>Save & Send via WhatsApp</span>
